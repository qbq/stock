...
(function(global) {
  ...
  seriesTypes.candlestick.prototype.getAttribs = function() {
    ...
    Highcharts.each(series.points, function (point) {
      ...
      if (point.open < point.close || (point.open === point.close && point.up)) {
  ...
  var klineOptions = {
    ...
    series: [{
      ...
      tooltip: {
        pointFormatter : function() {
          return '<span style="display:block; position: absolute; left: 20px; top: 25px; color: #8080ff;">MA5:'+this.y+'</span>';
        }
      }
    }, {
      type: 'spline',
      ...
      tooltip: {
        pointFormatter : function() {
          return '<span style="display:block; position: absolute; left: 40%; top: 25px; color: #ffcf88;">MA10:'+this.y+'</span>';
        }
      }
    }, {
      type: 'spline',
      ...
      tooltip: {
        pointFormatter : function() {
          return '<span style="display:block; position: absolute; right: 0; top: 25px; color: #ff8080;">MA30:'+this.y+'</span>';
        }
      }
    }],
    credits: false
        ...
        tickPositioner: function() {
          if (this.ordinalPositions) {
            ...
            var positions = this.ordinalPositions,
              ...
              interval = positions.length > 300 ? 60 : 30;

            $.each(positions, function(index, position) {

              // 保留不连续时间段的之前时间点
              if (index > 0 &&
                (positions[index] - positions[index - 1]) > 1 * 60 * 1000 &&
                ticks[ticks.length - 1] !== positions[index - 1] &&
                new Date(positions[index - 1]).getMinutes() % 30 === 0) {
                ticks.push(positions[index - 1]);
              }
            ...
            // mini化时，或者width较小时，每两个去除一个
            (mini || chartWidth < 500) && (ticks = $.grep(ticks, function(tick, index) {
              return index % 2 === 0;
            }));
      ...
      yAxis: [{
        ...
        labels: {
          ...
          formatter: function() {
            ...
            return '<span style="color: ' + color + '">' + Math.round(this.value * this.chart._dataPrecision) / this.chart._dataPrecision + '</span>';
          }
        },
        plotLines: [{
          width: 1,
        ...
        tickPositioner: function() {
          ...
          if (this.chart.positions) {
            ...
            // 最大最小值不存在，则以closePrice上涨10%和下跌10%作为最大最小值
            if (!max && !min) {
              max = closePrice * (1 + 0.1);
              min = closePrice * (1 - 0.1);
            }
  ...
  // 复权
  var splits = ['kline1Min', 'kline5Min', 'kline15Min', 'kline30Min', 'kline60Min', 'klineDay', 'klineWeek', 'klineMonth'];

  // MA指标
  var ma = ['klineDay', 'klineWeek', 'klineMonth'];
  ...
  function createTabBar(types) {
    ...
    $.each(types, function(index, type) {
      ...
      // 除权类型选择
      if (splits.indexOf(type) >= 0) {
        tabItem.append('<ul class="split-tabs">' +
          '<li class="split-tab">不复权</li>' +
          '<li class="split-tab" type="1">前复权</li>' +
          '<li class="split-tab" type="2">后复权</li>' +
          '</ul>');
      }
  ...
  var Chart = function(container, options) {
    ...
    options = options || {};
    self._dataPrecision = Math.pow(10, options.dataPrecision || 2);
    ...
    var types = options.types || ['min', 'kline1Min', 'kline5Min', 'kline15Min', 'kline30Min', 'kline60Min', 'klineDay', 'klineWeek', 'klineMonth'];

    // 如果types为多个则创建出tab栏做切换
    if (types.length > 1) {
      var tabBar = createTabBar(types);
      tabBar.on('click', function(event) {
        var selectTabItem = $(event.target).closest('li[id]');
        var type = selectTabItem.data('type');

        var splitType = $(event.target).closest('.split-tab').attr('type');
        if (type) {
          self.changeChart(type, splitType);
        }
      });
  ...
  Chart.prototype.changeChart = function(type, splitType) {
    var chartPanel = this._container.find('.panel').hide().filter('.' + type);

    if (chartPanel.length === 0 || chartPanel.data['splitType'] !== splitType) {
      if (type.indexOf('min') >= 0) {
        chartPanel = this.createMinChart(type);
      } else if (type.indexOf('kline') >= 0) {
        chartPanel.remove();
        chartPanel = this.createKlineChart(type, splitType);
      }
    }
    chartPanel.show();
  };
  ...
  Chart.prototype.createMinChart = function(type) {
    ...
    chartPanel.highcharts('StockChart', chartOption, function() {
      var chart = this;
      chart.mini = mini;
      chart._dataPrecision = self._dataPrecision;
