...
(function (global) {
  ...
  var defaultTimeInfo = (function() {
    var now = new Date();
    return {
      RiQi: [now.getFullYear(), (now.getMonth() + 1), now.getDate()].join(''),
      JiaoYiShiJianDuan: [
        {
          KaiShiShiJian: '0930',
          JieShuShiJian: '1130',
          KaiShiRiQi: [now.getFullYear(), (now.getMonth() + 1), now.getDate()].join(''),
          JieShuRiQi: [now.getFullYear(), (now.getMonth() + 1), now.getDate()].join('')
        },
        {
          KaiShiShiJian: '1300',
          JieShuShiJian: '1500',
  ...
  ChartDataProvider.prototype.subscribeData = function(type, done) {
    ...
    var updateMinData = function(time, price, volume, average) {
      var index = minTime.indexOf(time);
      if (index >= 0) {
        minPrice[index] = [time, price];
        minAverage[index] = [time, average];
        minVolume[index] = [time, volume];
      } else {

        // 没有对应时间时，认为交易日期有跨越，对应修正交易时间数据
        var date = new Date(time);

        var firstTime = minTime[0];
        var overDays = parseInt((time - firstTime) / oneDay);

        var overTime = overDays * oneDay;
        index = minTime.indexOf(time - overTime);

        // 找到跨越时间点将之后的数据统一修改日期到数据对应的日期然后重做更新数据操作
        if (index >= 0) {
          while(index < minTime.length) {
            var oldTime = minTime[index];
            oldTime += overTime;
            minVolume[index][0] = minPrice[index][0] = minAverage[index][0] = minTime[index] = oldTime;
            index++;
          }
          updateMinData(time, price, volume, average);
        }
      }
    };

    var initMinData = function(reverse, time, price, volume, average) {
      var func = reverse === true ? 'unshift' : 'push';
      minTime[func](time);
      minPrice[func]([time, price || null]);
      minVolume[func]([time, volume || null]);
      minAverage[func]([time, average || null]);
    };
    ...
    //  ZuoShou = data[self.obj]['ZuoShou'];
      ...
      dataStore.dataParser.direct = true;
      dataStore.subscribe({obj: self.obj}, function (data) {

        var timeInfo = defaultTimeInfo;
        if (data && data.length > 0) {
          data = data[0];
          data.JiaoYiShiJianDuan && (timeInfo = data);

          // 清盘
          if (timeInfo.QingPan === 1) {
            minTime = null;
          }
        }

        // 昨收可能会更新
        ZuoShou = timeInfo.ZuoShou || ZuoShou;
        ...
        if (!minTime) {
          ...
          if (times && times.length > 0) {
            ...
            times.forEach(function(eachTime, index) {
              var startTime = getTime(eachTime.KaiShiRiQi, eachTime.KaiShiShiJian),
              ...
              while(startTime <= endTime) {
                //if (startTime === 1451019600000) {
                //  startTime += oneMinute;
                //  continue;
                //}
                initMinData(false, startTime);
                startTime += oneMinute;
              }
              lastTime = endTime;
            });
            ...
            for(var i = 1; i <= prefixMinute; i++) {
              initMinData(true, startTime - (i * oneMinute));
            }
          }
        }
        //data = data[self.obj];
        ...
        if (data && data.length > 0) {
          $.each(data, function (i, eachData) {
            var time = eachData.ShiJian * 1000;
            updateMinData(time, eachData['ChengJiaoJia'] || null, eachData['ChengJiaoLiang'] || null, eachData['JunJia'] || null);
          })
        }
